...
export default function Home() {
  ...
  return (
    ...
    <div className="flex flex-col justify-center items-center gap-4 p-4">
      ...
      <div className="border p-4 rounded-md max-w-xl w-xl md:text-4xl text-lg">
        <p>{currentQuestion.date}회 출제 문제</p>
        <p className="md:text-5xl text-xl font-semibold">
          ...
          {currentQuestion.answers.map((answer, index) => (
            <li key={index} className="flex items-center mb-1">
              <button
                ...
                className={`bg-gray-200 hover:bg-gray-300 px-2 py-2 rounded mr-2 md:text-6xl text-2xl ${
                  activeButtonIndex === index ? "focus:outline-none focus:ring focus:ring-blue-300" : ""
                }`}
              >
                {index + 1}
              </button>
              <span className="md:text-5xl text-xl">
                {parse(answer)}
              </span>
            </li>
          ))}
